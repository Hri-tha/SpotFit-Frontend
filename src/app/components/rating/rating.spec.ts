...
export class RatingComponent implements OnInit {
  ...
  startRating() {
    ...
    if (!this.canRate && !this.userRating) {
      alert('You need to purchase this product to rate it');
      return;
    }
  ...
  submitRating() {
    ...
    this.productService.submitRating(this.productId, {
      ...
      orderId: this.userRating?.orderId || ''
    }).subscribe({
      next: (updatedProduct) => {
        ...
        this.userRating = {
          ...
          userName: this.userRating?.userName || '',
          rating: this.finalRating,
          review: this.tempReview,
          orderId: this.userRating?.orderId || '',
          createdAt: this.userRating?.createdAt || new Date()
        };
